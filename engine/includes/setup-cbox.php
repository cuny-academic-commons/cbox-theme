...
/**
 * Turn object buffering on when on certain parts of the CBOX Theme Options page.
 *
 * Most notably, during the output of <head> scripts and during the footer.
 * Part of the hotfix approach from cbox_jqueryui_hotfix().
 *
 * @since 1.0.9.1
 *
 * @see cbox_admin_remove_head_scripts()
 * @see cbox_admin_remove_footer_scripts()
 */
function cbox_admin_buffer_on() {
	if ( ! did_action( 'load-appearance_page_' . INFINITY_ADMIN_PAGE ) ) {
		return;
	}

	global $current_screen;
	$current_screen->cbox_theme_buffer = true;

	ob_start();
}
add_action( 'admin_print_scripts-appearance_page_' . INFINITY_ADMIN_PAGE, 'cbox_admin_buffer_on', 9999 );
add_action( 'admin_print_footer_scripts', 'cbox_admin_buffer_on', 0 );

/**
 * Removes WP's dynamic load-scripts.php javascript from <head>
 *
 ...
 * @since 1.0.9.1
 */
function cbox_admin_remove_head_scripts() {
	global $current_screen;

	if ( empty( $current_screen->cbox_theme_buffer ) ) {
		return;
	}
	...
	$load_scripts_url = admin_url( 'load-scripts.php' );

	$load_scripts_start = strpos( $contents, "<script type='text/javascript' src='{$load_scripts_url}" );
	$load_scripts_end   = strpos( $contents, '</script>', $load_scripts_start );

	echo substr_replace( $contents, '', $load_scripts_start, $load_scripts_end - $load_scripts_start + 9 );
}
 ...
 * @since 1.0.9.1
 */
function cbox_admin_remove_footer_scripts() {
	$contents = ob_get_contents();
	ob_end_clean();

	// might need to add more jquery UI declarations here
	...
	echo str_replace( $jquery_ui_scripts, '', $contents );
}
add_action( 'admin_footer-appearance_page_' . INFINITY_ADMIN_PAGE, 'cbox_admin_remove_footer_scripts' );
